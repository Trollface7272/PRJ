...
public class Hud : MonoBehaviour {
    ...
    private void Start() {
        ...
        for (var i = 0; i < 7; i++) { //Load vanity and armor elements
        ...
        _ttTxtTitle = toolTip.transform.GetChild(0).GetChild(0).GetComponent<TextMeshProUGUI>();
        _ttTxtDesc = toolTip.transform.GetChild(0).GetChild(1).GetComponent<TextMeshProUGUI>();
        
        
        Instance = GetComponent<Hud>();
        UpdateHud();
        ToggleInv();
    }
    ...
    public void UpdateHealth(float scale) {
        healthBar.SetValue(scale);
    }
    public void UpdateMana(float scale) {
        manaBar.SetValue(scale);
    }
